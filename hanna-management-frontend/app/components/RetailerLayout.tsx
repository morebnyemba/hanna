...
export default function RetailerLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-gray-100 font-sans">
      ...
      {/* Sidebar */}
      <aside className={`z-30 fixed inset-y-0 left-0 w-64 px-2 py-4 bg-indigo-800 text-white transition-all duration-300 ease-in-out transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 ${isSidebarCollapsed ? 'md:w-16' : 'md:w-64'} flex flex-col`}>
        <div className="flex items-center justify-between px-4 mb-6 shrink-0">
          <h2 className={`text-2xl font-semibold ${isSidebarCollapsed ? 'hidden md:block' : ''}`}>Retailer</h2>
        ...
        <div className="mt-4 px-2 pt-2 border-t border-indigo-700">
           <button onClick={handleLogout} className={`flex items-center w-full px-4 py-3 text-sm font-medium text-gray-300 rounded-md hover:bg-indigo-700 hover:text-white ${isSidebarCollapsed ? 'md:justify-center' : ''}`}>
      ...
      <div className="flex flex-col flex-1 w-full overflow-y-auto max-w-full overflow-x-hidden">
        <header className="z-10 py-4 bg-indigo-700 shadow-md">
          <div className="container flex items-center justify-between h-full px-6 mx-auto">
            ...
            <span className="font-semibold text-white md:hidden">Retailer Portal</span>
