...
LEAD_GENERATION_FLOW = {
    ...
    "steps": [
        ...
        {"name": "ask_delivery_phone", "type": "question", "config": {"message_config": {"message_type": "text", "text": {"body": "Great. What is the best contact number for the delivery?"}}, "reply_config": {"expected_type": "text", "save_to_variable": "delivery_phone", "validation_regex": "^\\+?[1-9]\\d{8,14}$"}, "fallback_config": {"action": "re_prompt", "max_retries": 2, "re_prompt_message_text": "Please enter a valid phone number (e.g., +263...)"}}, "transitions": [{"to_step": "ask_contact_email", "condition_config": {"type": "always_true"}}]},
        {"name": "ask_contact_email", "type": "question", "config": {"message_config": {"message_type": "text", "text": {"body": "What's the best email for your receipt and order updates?"}}, "reply_config": {"expected_type": "text", "save_to_variable": "contact_email", "validation_regex": "^.+@.+\\..+$"}, "fallback_config": {"action": "re_prompt", "max_retries": 2, "re_prompt_message_text": "Please provide a valid email (e.g., name@example.com)."}}, "transitions": [{"to_step": "ask_delivery_address", "condition_config": {"type": "always_true"}}]},
