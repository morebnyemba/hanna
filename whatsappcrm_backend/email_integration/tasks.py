...
def _create_order_from_invoice_data(attachment: EmailAttachment, data: dict, log_prefix: str):
    ...
    if isinstance(line_items, list):
        for item_data in line_items:
            ...
            if not product:
                logger.info(
                    f"{log_prefix} Created OrderItem without product for SKU '{product_code}' / '{product_description}'. "
        ...
        if customer_profile:
            ...
            queue_notifications_to_users(
                template_name='pfungwa_invoice_processed_successfully',
            ...
            queue_notifications_to_users(
                template_name='pfungwa_customer_invoice_confirmation', # New template for customers
...
def _create_job_card_from_data(attachment: EmailAttachment, data: dict, log_prefix: str):
    ...
    if serial_number:
        ...
        if not serialized_item:
            ...
            if product:
                # Create the new serialized item only if product exists
                ...
                # Don't create products automatically - SerializedItem will be null
                logger.info(
                    f"{log_prefix} Product not found for description '{product_description}'. "
                    f"SerializedItem not created for serial number '{serial_number}'. "
                    f"Link product manually to create SerializedItem."
                )
    ...
    if created:
        ...
        queue_notifications_to_users(
            template_name='pfungwa_job_card_created_successfully',
